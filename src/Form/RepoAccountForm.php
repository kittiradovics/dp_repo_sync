...
class RepoAccountForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['method'] = [
      ...
      '#description' => $this->t('This authentication method can be used in place of your original password (read <a href=":auth-info-link">more</a>).', [
        ':auth-info-link' => $this->config->get('dp_repo_sync.import')->get("help.auth.{$default_provider}"),
      ]),
    ];
  ...
  protected function applyConfigDefaults(FormStateInterface $form_state) {
    $this->restrict_methods = (bool) $this->config->get('dp_repo_sync.import')->get('account.restrict_methods');

    if (!$this->getEntity()->isNew()) {
      return;
    }

    $default_provider = $this->config->get('dp_repo_sync.import')->get('account.default_provider');
    if ($default_provider && !$form_state->getValue('provider')) {
      $form_state->setValue('provider', $default_provider);
    }

    $default_method = $this->config->get('dp_repo_sync.import')->get('account.default_method');
